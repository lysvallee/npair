...
if not os.path.exists(log_dir):
    try:
        ...
        log_dir = None  # Set log_dir to None or an alternative path if permission is denied
...
app.mount("/data", StaticFiles(directory="/home/ev/3Dgen/npair/data"), name="data")
...
async def choose_category(request: Request, selected_category: str = Form(...)):
    ...
    if not selected_category:
        raise HTTPException(status_code=422, detail="Please select at least one category.")
    selection_time = datetime.datetime.now().isoformat()
    # Post to data API
    async with httpx.AsyncClient() as client:
        response = await client.post(f"{DATA_API_URL}/usages", json={
...
async def images(request: Request, category: str, page: int = 1):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{DATA_API_URL}/images/{category}?page={page}&page_size={page_size}")
        response.raise_for_status()
        image_paths = response.json()
    logger.debug(f"image paths: {image_paths}")        
    return templates.TemplateResponse("images.html", {"request": request, "images": image_paths, "category": category, "current_page": page})
...
async def choose_image(request: Request, selected_image_path: str = Form(...)):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.put(f"{DATA_API_URL}/usages/latest", json={
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(timeout=3600)) as client:  # 1 hour timeout
        ...
        # Post to model API
        response = await client.post(f"{MODEL_API_URL}/generate", json={"image_path": image_path})
        if response.status_code != 200:
        ...
        mime_type, _ = mimetypes.guess_type(model_data['object_2d'])
        with open(model_data['object_2d'], "rb") as video_file:
            render_data = base64.b64encode(video_file.read()).decode("utf-8")
